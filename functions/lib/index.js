...
exports.adminManageBusinessInfo = exports.adminManageGallery = exports.adminManageReview = exports.createReview = exports.adminManageService = exports.upsertSchedule = exports.manageSession = exports.reorderQueue = exports.adminUpdateAppointment = exports.cancelAppointment = exports.bookAppointment = exports.adminManageUser = exports.updateProfile = exports.registerUser = void 0;
...
//  Helpers
// ──────────────────────────────────────────────
function generateTimeSlots(startTime, endTime, slotDurationMins) {
    const slots = [];
...
exports.adminManageUser = functions.https.onCall(async (data, context) => {
    ...
    switch (action) {
        case 'create': {
            ...
            return { id: userRecord.uid, ...doc.data(), createdAt: tsToString((_a = doc.data()) === null || _a === void 0 ? void 0 : _a.createdAt), updatedAt: tsToString((_b = doc.data()) === null || _b === void 0 ? void 0 : _b.updatedAt) };
        }
        case 'delete': {
            const { userId } = data;
            if (userId === adminUid)
        ...
        case 'deactivate': {
            const { userId } = data;
            if (userId === adminUid)
                throw new functions.https.HttpsError('failed-precondition', 'Cannot deactivate your own account');
            ...
            return { id: userId, ...updated.data(), createdAt: tsToString((_c = updated.data()) === null || _c === void 0 ? void 0 : _c.createdAt), updatedAt: tsToString((_d = updated.data()) === null || _d === void 0 ? void 0 : _d.updatedAt) };
        }
        case 'activate': {
            const { userId } = data;
            const userDoc = await db.collection('users').doc(userId).get();
...
exports.bookAppointment = functions.https.onCall(async (data, context) => {
    ...
    if (!date || !timeSlot)
        throw new functions.https.HttpsError('invalid-argument', 'Date and time slot are required');
    if (!/^\d{4}-\d{2}-\d{2}$/.test(date))
        throw new functions.https.HttpsError('invalid-argument', 'Date must be YYYY-MM-DD');
    if (!/^([01]\d|2[0-3]):[0-5]\d$/.test(timeSlot))
        throw new functions.https.HttpsError('invalid-argument', 'Time slot must be HH:MM (24h)');
    const today = getTodayString();
    if (date < today)
        throw new functions.https.HttpsError('failed-precondition', 'Cannot book appointments in the past');
    // Check user is active
    const userDoc = await db.collection('users').doc(uid).get();
    if (!userDoc.exists || !((_a = userDoc.data()) === null || _a === void 0 ? void 0 : _a.isActive)) {
    ...
    const result = await db.runTransaction(async (transaction) => {
        ...
        if (!scheduleDoc.exists)
            throw new functions.https.HttpsError('not-found', 'No schedule found for this date');
        const schedule = scheduleDoc.data();
        if (schedule.status !== 'OPEN')
            throw new functions.https.HttpsError('failed-precondition', 'This date is not available for booking');
        // 2. Session must not be closed
        if (sessionDoc.exists && ((_a = sessionDoc.data()) === null || _a === void 0 ? void 0 : _a.isClosed)) {
        ...
        if (!validSlots.includes(timeSlot))
            throw new functions.https.HttpsError('invalid-argument', 'Invalid time slot for this date');
        ...
        if (userDayLockDoc.exists)
            throw new functions.https.HttpsError('already-exists', 'You already have an active booking on this date');
        // 6. Queue position
...
exports.cancelAppointment = functions.https.onCall(async (data, context) => {
    ...
    if (appointment.status === 'COMPLETED')
        throw new functions.https.HttpsError('failed-precondition', 'Cannot cancel a completed appointment');
    if (appointment.status === 'CANCELLED')
        throw new functions.https.HttpsError('failed-precondition', 'Appointment is already cancelled');
    if (appointment.status === 'NO_SHOW')
        throw new functions.https.HttpsError('failed-precondition', 'Cannot cancel a no-show appointment');
    const today = getTodayString();
    if (appointment.date < today)
        throw new functions.https.HttpsError('failed-precondition', 'Cannot cancel a past appointment');
    ...
    return {
        ...
        serviceId: (_b = appointment.serviceId) !== null && _b !== void 0 ? _b : null,
        status: 'CANCELLED',
        createdAt: tsToString(appointment.createdAt),
        updatedAt: new Date().toISOString(),
    };
});
exports.adminUpdateAppointment = functions.https.onCall(async (data, context) => {
    var _a;
    ...
    switch (apptAction) {
        case 'complete': {
            ...
            if (appointment.status === 'CANCELLED')
                throw new functions.https.HttpsError('failed-precondition', 'Cannot complete a cancelled appointment');
            const batch = db.batch();
        ...
        case 'in-service': {
            if (appointment.status !== 'BOOKED')
                throw new functions.https.HttpsError('failed-precondition', 'Only booked appointments can be marked in service');
        ...
        case 'update': {
            ...
            if (data.timeSlot)
                updates.timeSlot = data.timeSlot;
            if (data.date)
    ...
    return {
        ...
        serviceId: (_a = d.serviceId) !== null && _a !== void 0 ? _a : null,
        status: d.status,
...
//  QUEUE
// ──────────────────────────────────────────────
exports.reorderQueue = functions.https.onCall(async (data, context) => {
    await assertAdmin(context);
...
//  SESSION
// ──────────────────────────────────────────────
exports.manageSession = functions.https.onCall(async (data, context) => {
    var _a, _b, _c, _d;
    await assertAdmin(context);
    ...
    switch (sessionAction) {
        case 'open': {
            ...
            if (!scheduleDoc.exists || ((_a = scheduleDoc.data()) === null || _a === void 0 ? void 0 : _a.status) !== 'OPEN') {
            ...
            return {
                ...
                isClosed: false,
                exists: true,
                openedAt: tsToString((_b = session.data()) === null || _b === void 0 ? void 0 : _b.openedAt),
                closedAt: null,
            };
        }
        case 'close': {
            ...
            const active = await db.collection('appointments')
                ...
                .get();
            if (!active.empty) {
                throw new functions.https.HttpsError('failed-precondition', `Cannot close session: ${active.size} active appointment(s) remaining`);
            }
            ...
            return {
                ...
                isClosed: true,
                exists: true,
                openedAt: tsToString((_c = session.data()) === null || _c === void 0 ? void 0 : _c.openedAt),
                closedAt: tsToString((_d = session.data()) === null || _d === void 0 ? void 0 : _d.closedAt),
            };
        }
        default:
...
//  SCHEDULE
// ──────────────────────────────────────────────
exports.upsertSchedule = functions.https.onCall(async (data, context) => {
    var _a, _b;
    await assertAdmin(context);
    ...
    if (existing.exists) {
        ...
        if (!activeBookings.empty) {
            if (((_a = existing.data()) === null || _a === void 0 ? void 0 : _a.slotDurationMins) !== slotDurationMins && !forceSlotChange) {
                throw new functions.https.HttpsError('failed-precondition', `Cannot change slot duration: ${activeBookings.size} active booking(s) exist. Set forceSlotChange=true to confirm.`);
            }
            if (status !== 'OPEN' && ((_b = existing.data()) === null || _b === void 0 ? void 0 : _b.status) === 'OPEN') {
                throw new functions.https.HttpsError('failed-precondition', `Cannot close this date: ${activeBookings.size} active booking(s) exist. Cancel them first.`);
            }
        }
    }
    const scheduleData = {
...
// ──────────────────────────────────────────────
exports.adminManageService = functions.https.onCall(async (data, context) => {
    var _a, _b, _c, _d;
    await assertAdmin(context);
    ...
    switch (svcAction) {
        case 'create': {
            ...
            return { id: ref.id, ...doc.data(), createdAt: tsToString((_a = doc.data()) === null || _a === void 0 ? void 0 : _a.createdAt), updatedAt: tsToString((_b = doc.data()) === null || _b === void 0 ? void 0 : _b.updatedAt) };
        }
        case 'update': {
            ...
            const doc = await ref.get();
            return { id: serviceId, ...doc.data(), createdAt: tsToString((_c = doc.data()) === null || _c === void 0 ? void 0 : _c.createdAt), updatedAt: tsToString((_d = doc.data()) === null || _d === void 0 ? void 0 : _d.updatedAt) };
        }
        case 'delete': {
...
exports.createReview = functions.https.onCall(async (data, context) => {
    ...
    if (!appointmentId || !rating)
        throw new functions.https.HttpsError('invalid-argument', 'Appointment ID and rating are required');
    ...
    if (appointment.userId !== uid)
        throw new functions.https.HttpsError('permission-denied', 'You can only review your own appointments');
    if (appointment.status !== 'COMPLETED')
        throw new functions.https.HttpsError('failed-precondition', 'Can only review completed appointments');
    ...
    if (!existingReview.empty)
        throw new functions.https.HttpsError('already-exists', 'A review already exists for this appointment');
    const now = admin.firestore.FieldValue.serverTimestamp();
    ...
    return {
        id: ref.id,
        ...doc.data(),
        createdAt: tsToString((_a = doc.data()) === null || _a === void 0 ? void 0 : _a.createdAt),
        updatedAt: tsToString((_b = doc.data()) === null || _b === void 0 ? void 0 : _b.updatedAt),
        user: {
            id: uid,
            firstName: (_c = userDoc.data()) === null || _c === void 0 ? void 0 : _c.firstName,
            lastName: (_d = userDoc.data()) === null || _d === void 0 ? void 0 : _d.lastName,
            profileImageUrl: (_e = userDoc.data()) === null || _e === void 0 ? void 0 : _e.profileImageUrl,
        },
        appointment: {
            id: appointmentId,
            date: appointment.date,
...
exports.adminManageReview = functions.https.onCall(async (data, context) => {
    var _a, _b, _c, _d, _e;
    await assertAdmin(context);
    ...
    switch (reviewAction) {
        ...
        case 'toggleVisibility': {
            ...
            return {
                id: reviewId,
                ...updated.data(),
                createdAt: tsToString((_a = updated.data()) === null || _a === void 0 ? void 0 : _a.createdAt),
                updatedAt: tsToString((_b = updated.data()) === null || _b === void 0 ? void 0 : _b.updatedAt),
                user: userDoc.exists ? {
...
//  GALLERY
// ──────────────────────────────────────────────
exports.adminManageGallery = functions.https.onCall(async (data, context) => {
    var _a, _b, _c, _d;
    await assertAdmin(context);
    ...
    switch (galleryAction) {
        case 'create': {
            ...
            if (!title || !imageUrl)
                throw new functions.https.HttpsError('invalid-argument', 'Title and imageUrl are required');
            const now = admin.firestore.FieldValue.serverTimestamp();
        ...
        case 'update': {
            ...
            delete updates.action;
            if (!galleryId)
                throw new functions.https.HttpsError('invalid-argument', 'Gallery item ID is required');
            ...
            await ref.update(clean);
            const doc = await ref.get();
            return { id: galleryId, ...doc.data(), createdAt: tsToString((_c = doc.data()) === null || _c === void 0 ? void 0 : _c.createdAt), updatedAt: tsToString((_d = doc.data()) === null || _d === void 0 ? void 0 : _d.updatedAt) };
        }
        case 'delete': {
...
//  BUSINESS INFO
// ──────────────────────────────────────────────
exports.adminManageBusinessInfo = functions.https.onCall(async (data, context) => {
    var _a, _b, _c, _d;
    await assertAdmin(context);
    ...
    switch (biAction) {
        case 'upsert': {
            ...
            if (!key || !value)
                throw new functions.https.HttpsError('invalid-argument', 'Key and value are required');
            ...
            return { id: doc.id, key, ...doc.data(), createdAt: tsToString((_a = doc.data()) === null || _a === void 0 ? void 0 : _a.createdAt), updatedAt: tsToString((_b = doc.data()) === null || _b === void 0 ? void 0 : _b.updatedAt) };
        }
        case 'bulkUpsert': {
            const { items } = data;
            ...
            for (const item of items) {
                ...
                results.push({ id: doc.id, key: item.key, ...doc.data(), createdAt: tsToString((_c = doc.data()) === null || _c === void 0 ? void 0 : _c.createdAt), updatedAt: tsToString((_d = doc.data()) === null || _d === void 0 ? void 0 : _d.updatedAt) });
            }
            return results;
        }
        case 'remove': {
