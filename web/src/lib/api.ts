...
// ─── In-memory session (primary) + localStorage (persistence for reloads) ──

let inMemorySession: SessionState | null = null
...
function getStoredSession(): SessionState | null {
  ...
  if (!raw) return null
  try {
    inMemorySession = JSON.parse(raw) as SessionState
