...
These are injected into the Android CI workflow so the app can compile with production Firebase config.

### 4. One-command Firebase deploy (local)
...
This deploys Firestore rules/indexes, Cloud Functions, and Hosting using existing `firebase.json` and `.firebaserc`.
